...
> [!IMPORTANT]
> Данная программа предназначена для сборки и запуска на устройствах с POSIX-совместимыми операционными системами, работающими на процессорах с архитектурой intel x86_64 tigerlake (тесты проводились на процессоре intel i7-1165g7 (тактовая частота 2.8 ГГц) и arch linux)
...
Одной из величин, характеризующих состояние хеш-таблицы, является **коэффициент заполнения** (англ. load factor), рассчитывающийся, как частное количества элементов в таблице и количества ячеек. От него напрямую зависит скорость поиска элемента, однако не стоит забывать, что **load factor отражает лишь среднее значение заселенности таблицы**. Это значение является валидным лишь в случае идеально равномерного распределения, так как представляет из себя математическое ожидание количества элементов в ячейке в этой ситуации. Таким образом, скорость поиска элемента зависит не только от **load factor**, но и от равноемерности распределения значений в таблице.
...
В тестах использовалась хеш-таблица, хранящая структуры, каждая из которых состоит из строки и ее длины. В качесте датасета был взят текст книги "Преступление и наказание", предварительно очищеный от всех знаков препинания (кроме апострофов) и приведенный к нижнему регистру (9576 уникальных слов). Размер таблицы (`1499` ячеек) был подобран так, чтобы load factor составлял `~6-7` элементов на ячейку. Число `1499` выбрано т.к. оно простое, а значит размер таблицы гарантировано не будет кратен периоду хеш-функции, что позволяет избежать случаев, когда пики распределения значений хеша накладываются друг на друга.
